...
* If you're on the master branch or v12, you'll need to use `Discord.MessageEmbed()`, not `Discord.RichEmbed()`.
...
* The `color` field **must** be a base 10 integer. If you have a hex color (`#0099ff`, for example), you can replace the `#` with `0x` and use that (as a number).

#### General embed notes

* You need at least two consecutive fields set to `inline` in order for them to display correctly.
* If you have a thumbnail set, a maximum of two fields will be displayed inlined, as opposed to the default 3.
