...
Then, you simply register the command again:
```sh:no-line-numbers
...
So far, you already have the Guild ID, so:

- Command ID ✅
- Guild ID
- User/role ID

With these 3 IDs, you make another HTTP PUT request but to a different endpoint (route). As you might've guessed, that means we need to go back to your `deploy-commands.js` file to add a few more lines:
...
As you can see in line 16, calling `rest.put()` to this specific endpoint `Routes.applicationGuildCommands()` will return a response. You store that response in the variable `response`. This response will be an array of all the registered commands.

Then, in line 23, you retrieve the `ping` command from that array with [`Array.find()`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/find). Once you retrieve the `ping` command, you can access its ID from the `id` property.
...
Finally, you make the HTTP PUT request in line 38. However, you use a different endpoint (route) this time: `Routes.guildApplicationCommandsPermissions()` (line 39), and you pass your array in the body of the PUT request (line 40).
