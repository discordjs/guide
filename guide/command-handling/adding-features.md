...
In the DM that the bot will be sending the user, the bot will automatically retrieve the name of the command, the bot's prefix, and the description from the individual command files that you created earlier and the bot's `config.json` file. (this is all done in the second `data.push` line). It will then place the name of the command in a code snippet to make the command stand out and append the description of the command after the name of the command so that the user can better understand it at a glance.
