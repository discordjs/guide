...
You can use `npm run dev` to open up a local version of the site at [http://localhost:8080](http://localhost:8080). If you need to use a different port, run it as `npm run dev -- --port=1234`.
...
`````js {2,6-8} client.once('ready', () => { console.log(```${client.user.tag} ready.\`\); }\);
